...
for rp in required_programs:
    if not is_executable_in_path(rp):
        if _error == 0:
            print("=========================== ERROR WHILE TRYING TO INSTALL hpcservice ===========================")
...
# Warn if we are installing over top of an existing installation. This can cause issues where files that were deleted from a more recent hpcservice are still present in site-packages. See #18115.
...
if "install" in sys.argv:
    ...
    for lib_path in lib_paths:
        existing_path = os.path.abspath(os.path.join(lib_path, "hpcservice"))
...
setup(
    ...
    name='hpcservice',
    version='0.01',
    description='Allows you to use a Slurm-based HPC-Cluster for constantly running services',
    author='Norman Koch',
...
You have just installed hpcservice over top of an existing
installation, without removing it first. Because of this,
your install may now include extraneous files from a
previous version that have since been removed from
hpcservice. This is known to cause a variety of problems. You
should manually remove the

%(existing_path)s

directory and re-install hpcservice.
